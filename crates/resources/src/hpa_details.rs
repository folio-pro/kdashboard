...
impl HpaDetails {
    ...
    pub fn on_action(
        mut self,
        handler: impl Fn(HpaAction, &mut Context<'_, Self>) + 'static,
    ) -> Self {
...
impl HpaDetails {
    ...
    fn render_breadcrumb(&self, cx: &Context<'_, Self>) -> impl IntoElement {
        ...
        div()
            ...
            .min_w(px(0.0))
            .child(
                back_btn("hpa-details-back-btn", colors).on_click(cx.listener(
                    |this, _, _window, cx| {
    ...
    fn render_header(&self, cx: &Context<'_, Self>) -> impl IntoElement {
        ...
        let replicas_text = format!(
            "{}/{} replicas (min: {}, max: {})",
            current, desired, min, max
        );

        let is_stable = current == desired;
        let status_text = if is_stable {
            "Stable"
    ...
    fn render_scaling_card(&self, cx: &Context<'_, Self>, resource: &Resource) -> impl IntoElement {
        ...
        let progress = if max > min {
            ((current - min) as f32 / (max - min) as f32 * 100.0)
                .min(100.0)
                .max(0.0)
        } else {
            0.0
    ...
    fn render_metrics_card(&self, cx: &Context<'_, Self>, resource: &Resource) -> impl IntoElement {
        ...
        let metric_items: Vec<Div> = spec_metrics
            .iter()
            .enumerate()
            .map(|(idx, metric)| {
                ...
                let (name, target, current_val) = if metric_type == "Resource" {
                    ...
                    let label = if res_name == "cpu" {
                        "CPU"
                    } else if res_name == "memory" {
                        "Memory"
                    } else {
                        res_name
                    };
