...
pub struct ReplicaSetDetails {
    resource: Resource,
    scroll_handle: ScrollHandle,
    on_close: Option<Box<dyn Fn(&mut Context<'_, Self>) + 'static>>,
    on_action: Option<Box<dyn Fn(ReplicaSetAction, &mut Context<'_, Self>) + 'static>>,
...
impl Render for ReplicaSetDetails {
...
impl ReplicaSetDetails {
    ...
    fn render_header(&self, cx: &Context<'_, Self>) -> impl IntoElement {
        ...
        let ready = resource
            ...
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        let replicas_text = format!("{}/{} replicas", ready, desired);
