...
mod tests {
    ...
    fn theme_colors_from_mode_works_for_all_variants() {
        ...
        for mode in modes {
            let colors = ThemeColors::from_mode(mode);
            // Every theme should have an opaque background (alpha = 1.0)
            assert_eq!(colors.background.a, 1.0, "{:?} background alpha", mode);
            // Text color should always be opaque
    ...
    fn dark_themes_have_dark_backgrounds() {
        ...
        for mode in dark_modes {
            let colors = ThemeColors::from_mode(mode);
            // Dark themes should have low lightness background (< 0.3)
            assert!(
    ...
    fn light_themes_have_light_backgrounds() {
        ...
        for mode in light_modes {
            let colors = ThemeColors::from_mode(mode);
            // Light themes should have high lightness background (> 0.8)
            assert!(
    ...
    #[test]
    fn highlight_theme_returns_different_themes_for_dark_and_light() {
        let dark = highlight_theme_for(ThemeMode::EverforestDark);
        let light = highlight_theme_for(ThemeMode::EverforestLight);
        // The dark and light themes should have different names or appearances
